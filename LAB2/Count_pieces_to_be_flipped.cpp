...
int countFlipPieces(char board[][N], int next_player, string key, int direction_num) {
    int flip_num = 0;
...
int main() {
    ...
    cin >> cas;
    for (int i = 0; i < cas; ++i) {
        cin >> s >> next_player >> key >> direction_num;
        init(s);
        cout << countFlipPieces(board, next_player, key, direction_num) << "\n"[i == cas - 1];
    }
