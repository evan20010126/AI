...
int main() {
    ...
    if (!is_solvability(s)) {
        ...
        int number = 0;
        node* mynode = new node(s, "", 0, manhattan_distance(s), number++, NULL);
        ...
        while (!heap.empty()) {
            ...
            for (auto step : posible_step) {
                node* tmp = new node(step.second, step.first, mynode1->g + 1,
                                     manhattan_distance(step.second), number++, mynode1);
        ...
        if (!is_complete) {
            ...
            for (int i = action_buffer.size() - 1; i >= 0; --i)
                cout << "move 0 to " << action_buffer[i] << "\n"[i == 0];
        }
    }
    return 0;
}