    ...
    if (depth == 0) {
        ...
        for (char i = 'A'; i < 'G'; ++i) {
            for (char j = 'a'; j < 'g'; ++j) {
                ...
                if (first_player == 2) {
                    order.push_back(
                        {key, countColorPieces(new_board, 2) - countColorPieces(new_board, 1)});
                } else {
                    order.push_back(
                        {key, countColorPieces(new_board, 1) - countColorPieces(new_board, 2)});
                };
            }
        }

        sort(order.begin(), order.end(), compare);
        // for (auto item : order) { cout << item.first << " " << item.second << endl; }
